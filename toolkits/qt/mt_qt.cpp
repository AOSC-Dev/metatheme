...
static Q3PtrDict<void *> widgetRefs;
static Q3PtrDict<void> widgetData;
static Q3PtrDict<void> widgetDataDispose;
...
void MetaThemeStyle::unPolish(QWidget *widget)
{
   ...
   if (mt_engine->metric[MT_NOTEBOOK_IS_FILLED]) {
      if (!qstrcmp(widget->name(), "tab pages")) {
         ...
         Q3PtrDictIterator<QWidget> it(tabWidgets);
         while (it.current()) {
                              ...
                              const QStyleOption& opt) const
{
   ...
   switch (element) {
...
#if QT_VERSION >= 0x030300
         if (!styleHint(SH_UnderlineAccelerator, widget, QStyleOption::Default, 0)) alignment |= Qt::TextHideMnemonic;
#endif
      ...
      case CE_PopupMenuItem:
      {
         ...
         if (mi->iconSet()) {
            QIcon::Mode mode = (state & MT_DISABLED)? QIcon::Disabled : QIcon::Normal;
            if ((state & MT_ACTIVE) && !(state & MT_DISABLED)) mode = QIcon::Active;
            QPixmap pixmap;
...
#if QT_VERSION >= 0x030300
            if (!styleHint(SH_UnderlineAccelerator, widget)) text_flags |= Qt::TextHideMnemonic;
#endif
      ...
      case CE_ProgressBarLabel:
      {
         ...
         drawItem(p, r, Qt::AlignCenter | Qt::TextSingleLine, cg, flags & Style_Enabled, 0, pb->progressString(), -1, &cg.highlightedText());

         r2.setX(r.x() + progress);
         r2.setWidth(r.width() - progress);
         p->setClipRect(r2);
         MT_STRING_SET_TYPE(MT_PROGRESSBAR, state);
         drawItem(p, r, Qt::AlignCenter | Qt::TextSingleLine, cg, flags & Style_Enabled, 0, pb->progressString(), -1, &cg.foreground());
      ...
      case CE_PushButtonLabel:
      {
         ...
         drawItem(p, QRect(x, y, w, h), Qt::AlignCenter|Qt::TextShowMnemonic, button->colorGroup(), button->isEnabled(), button->pixmap(), button->text(), -1, &button->colorGroup().buttonText());
...
void MetaThemeStyle::drawControlMask(ControlElement element,
                              ...
                              const QStyleOption& opt) const
{
   QColorGroup cg(Qt::color1, Qt::color1, Qt::color1, Qt::color1, Qt::color1, Qt::color1, Qt::color1, Qt::color1, Qt::color0);
                                     ...
                                     const QStyleOption& opt) const
{
   ...
   switch(control) {
      ...
      case CC_SpinWidget:
      {
         const Q3SpinWidget *sw = (const Q3SpinWidget *)widget;
         SFlags flags;
         PrimitiveElement pe;
      ...
      case CC_ToolButton:
      {
         ...
         QColorGroup c = cg;
         if (toolbutton->backgroundMode() != Qt::PaletteButton) {
                                    ...
                                    const QStyleOption &opt) const
{
   switch (t) {
      ...
      case CT_PopupMenuItem:
      {
         ...
         if (mi->iconSet()) {
            QPixmap pixmap = mi->iconSet()->pixmap(QIcon::Small, QIcon::Normal, QIcon::On);
            h = pixmap.height();
            if (h <= 18) h -= 3;
         }
         else {
...
bool MetaThemeStyle::eventFilter(QObject *obj, QEvent *ev)
{
   ...
   if (obj->inherits("QFrame") && !obj->inherits("TEWidget") && ev->type() == QEvent::Paint) {
      Q3Frame *f = static_cast<Q3Frame*>(obj);
      ...
      if (f->frameShape() != Q3Frame::HLine && f->frameShape() != Q3Frame::VLine && f->frameShape() != Q3Frame::Panel && f->frameShape() != Q3Frame::WinPanel) {
         return false;
      }

      if ((f->frameShape() == Q3Frame::Panel || f->frameShape() == Q3Frame::WinPanel) && f->lineWidth() == 1) {
         return false;
      }
      ...
      int type;
      switch (f->frameShape()) {
         case Q3Frame::HLine: type = MT_HLINE; break;
         case Q3Frame::VLine: type = MT_VLINE; break;

         case Q3Frame::Panel:
         case Q3Frame::WinPanel:
            type = (f->frameShadow() == Q3Frame::Raised)? MT_BORDER_OUT : MT_BORDER_IN;
            break;
